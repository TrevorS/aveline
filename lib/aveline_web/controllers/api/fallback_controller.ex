defmodule AvelineWeb.Api.FallbackController do
  ...
  def call(conn, {:error, :not_notebook}),
    do:
      err(
        conn,
        422,
        "not_notebook",
        "Cell execution is only available on notebooks. This doc has kind \"doc\" — create a new one with kind: \"notebook\"."
      )

  def call(conn, {:error, :cell_not_found}),
    do:
      err(
        conn,
        404,
        "cell_not_found",
        "No runnable cell (frame or elixir code) with that block id in the doc's current version."
      )

  def call(conn, {:error, :invalid_actor}),
    do: err(conn, 422, "invalid_actor", "Actor must be \"human\" or \"agent\".", %{field: "actor"})

  def call(conn, {:error, :execution_disabled}),
    do:
      err(
        conn,
        422,
        "execution_disabled",
        "Code cells only execute on local-mode deployments (DEPLOY_MODE=local). This deployment renders them read-only — frame cells still run everywhere."
      )
